...
class MRSysSim:
    ...
    def runSystem(self): 
        ...
        #[[TODO:PartII.A]]  
        #BEGIN[Google][https://www.pylenin.com/blogs/stop-iteration-exception/]"StopIteration Exception in Python"
        to_reduce_task = [[] for i in range(self.num_reduce_tasks)]
        namenode_m2r_iter = iter(namenode_m2r)
        while True:
            try:
                rtask_num, rkv = next(namenode_m2r_iter)
                to_reduce_task[rtask_num].append(rkv)
...
class matrixMultMRSys(MRSysSim):
    ...
    def reduce(self, k, vs):      
        #[[TODO:PartII.B]]
        #BEGIN[Google][https://www.w3schools.com/python/ref_dictionary_setdefault.asp]"setdefault in dictionary in python"
        rowcolSum = 0
        rtvaluematrix = {}
        for valuematrix, rtt, vt in vs:
            rtvaluematrix.setdefault(rtt, {})[valuematrix] = vt
        #END[Google]
        rowcolSum = sum(rtvaluematrix[tj].get('m', 0) * rtvaluematrix[tj].get('n', 0) for tj in rtvaluematrix)
...
class meanMRSys(MRSysSim): 
    ...
    def map(self, k, v): 
        #[TODO]#
        rt, gt, bt = v
        #BEGIN[Google][https://developers.google.com/edu/python/dict-files]"Dictonary values with pair python"
        key_values = {
            'red': (rt, 1),
            'green': (gt, 1),
            'blue': (bt, 1)
        ...
        #END[Google]
        return [(colourrt, key_values[colourrt]) for colourrt in key_values]
    
    def reduce(self, k, vs): 
        #[TODO]#
        #BEGIN[Google][https://numpy.org/doc/stable/reference/generated/numpy.sum.html]"How to use Np sum"
        value_array = np.array(vs)
        gsumt = np.sum(value_array[:,0])
        gcount = np.sum(value_array[:,1])
        #END[Google]
        meanValue = gsumt / gcount if gcount > 0 else 0
        return (k, meanValue)
